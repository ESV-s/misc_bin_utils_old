...
enum {
	...
	///\n
/// Используется в основном цикле разборки: адрес инструкции уже посещен.
FLAG_VISITED = 1 << 0,

///\n
/// Это инструкция потока, и она продолжает выполнение следующей (в отличие от jmps или call).\n
/// TODO(soerenme) Как заметил olonho@, этот флаг используется несколько непоследовательно во всей кодовой базе.\n
/// Нам следует использовать следующее определение: если флаг установлен,\n
/// то выполнение гарантированно переходит к следующей инструкции.\n
/// Т.е. ветвления и переключения не имеют его установки. Вызовы могут быть, а могут и не быть,\n
/// в зависимости от того, возвращаются ли они.
FLAG_FLOW = 1 << 1,

///\n
/// Эта инструкция уже экспортирована (используется в программе написания базы данных).
FLAG_EXPORTED = 1 << 2,

		///\n
/// Эта инструкция является ветвью, условной или безусловной.\n
/// В настоящее время устанавливается только Bea.\n
/// Намеренно имеет то же значение, что и FLAG_EXPORTED, поскольку оба они используются в изолированных\n
/// и независимых контекстах и могут быть сброшены/забыты после использования.\n
FLAG_BRANCH = 1 << 2,

		///\n
/// Инструкция соответствовала подписям в библиотеке.
FLAG_LIB = 1 << 3,

		///\n
/// Эта инструкция недействительна.
FLAG_INVALID = 1 << 4,

		///\n
/// Эта инструкция является вызовом.
FLAG_CALL = 1 << 5,

		///\n
/// По этому адресу начинается или switch таблица или vtable.
FLAG_TABLE_START = 1 << 6,

		///\n
/// Эта инструкция является инструкцией NOP.
FLAG_NOP = 1 << 7,

		///\n
/// Эта инструкция является точкой входа.
...
#pragma pack(pop)
		...
		///\n
/// TODO(cblichmann): Удалите этот хак. Он находится здесь потому, что IDA определяет одноименную глобальную переменную.
namespace detego {
