...
class Expression {
 ...
 private:
  ...
  ///\n
  /// Конструктор является приватным по двум причинам:\n
  /// - Мы хотим убедиться, что выражения могут быть созданы только на куче.\n
  /// - Мы хотим избежать создания дубликатов выражений, поэтому мы просто\n
  ///	обращаемся к expression_cache_, если кто-то попытается это сделать\n
  ///	(таким образом происходит сжатие/дедупирование архивов)\n
