...
class EntryPoint {
public:
	// Возможные источники для точек входа.
	// - Вероятнее всего имеется ввиду не входа, а перехода в данную точку (функцию)
	enum class Source {
		CODE_FLOW = 0,
		CALL_TARGET,
		JUMP_ANY,
...
///\n
/// EntryPointManager не является потокобезопасным.
