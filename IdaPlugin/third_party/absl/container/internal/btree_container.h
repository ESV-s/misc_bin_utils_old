...
namespace absl {
	ABSL_NAMESPACE_BEGIN
		...
		///\n
		/// Общий базовый класс для btree_set, btree_map, btree_multiset и btree_multimap.
		...
		protected:
			///\n
			 /// Псевдоним, используемый для разнородных функций поиска.\n
			 /// `key_arg<K>` оценивается в `K`, если функторы прозрачны, и в `key_type` в противном случае.\n
			 /// Он разрешает вычитание шаблонных аргументов на `K` для прозрачного случая.
			template <class K>
			using key_arg =
		...
		public:
			...
			///\n
			/// Удаление указанного итератора из дерева btree. Итератор должен быть действительным (т.е. не равным end()).\n
			/// Возвращаем итератор, указывающий на узел, следующий за стертым (или end(), если такового не существует).
			...
			/// Компаратор ключей, используемый деревом btree.
		...
		/// Общий базовый класс для btree_set и btree_map.
		template <typename Tree>
