...
// [EFFECTS] --- begin ---
//! \brief \n Агрегаты побочных эффектов по функции. \n
/// \n\n
/// \ingroup FUNCTION_W
...
/// \brief \n Кол-во «событий первого касания»: число аллокаций, у которых был хотя бы один mem-доступ. \n
...
/// \brief \n Число аллокаций без локального первого касания (alloc_calls - heap_first_touch_events, не < 0). \n
	uint64_t alloc_no_first_touch = 0;


/// \brief \n Баланс аллок/фри: alloc_calls - free_calls (может быть отрицательным). \n
	int64_t  alloc_free_balance = 0;


/// \brief \n Доля «использованных» аллокаций в процентах (0..100), округление вниз. \n
/// \details 100 * heap_first_touch_events / max(1, alloc_calls).
	uint32_t alloc_use_rate_pct = 0;
	// [EFFECTS+DERIVED] --- end ---

	// === Wrapper/Forward/Dispatch flags ===
/// \brief \n Всего вызовов внутри функции (прямых+косвенных).  \n
	uint32_t calls_total = 0;
/// \brief \n Кол-во прямых вызовов (near/far).  \n
	uint32_t direct_calls = 0;
/// \brief \n Кол-во косвенных вызовов (через регистр/память).  \n
	uint32_t indirect_calls = 0;

/// \brief \n Обёртка: функция делает ровно один вызов и почти не имеет побочных эффектов.  \n
	bool is_wrapper = false;
/// \brief \n Tailcall/thunk-поведение: последний прыжок/вызов без дальнейшего кода.  \n
	bool is_tailcall_thunk = false;
/// \brief \n Форвардинг параметров «как есть» (эвристика).  \n
	bool forwards_params_only = false;
/// \brief \n Диспетчер: вызывает по указателю функции, вероятно взятому из параметра.  \n
	bool dispatches_via_funptr = false;

/// \brief \n Возвращает heap-указатель в RAX/EAX (после alloc).  \n
	bool returns_heap_ptr = false;
/// \brief \n Пишет heap-указатель в out-параметр (store через адрес параметра).  \n
...
/// \brief \n Класс для сбора информации о исполняемом файле , \n
...
/// \n\n
/// \ingroup FUNCTION_W INSTRUCTION_W
...
/// \brief \n Инструкция читает данные со стека (операнд через SP/BP/RSP/RBP). \n
...
/// \brief \n Инструкция пишет данные в стек. \n
/// \details Эвристика: как выше, но с флагом CF_CHG* для операнда.
		bool writes_stack = false;


/// \brief \n Инструкция читает глобальную/статическую память (о_mem или адрес из .data/.rdata). \n
		bool reads_global = false;


/// \brief \n Инструкция пишет в глобальную/статическую память. \n
		bool writes_global = false;


/// \brief \n Изменение стека (SP-дельта) на этой инструкции. \n
...
/// \brief \n Признак «работы с кучей» по месту: чтение/запись по указателю, полученному из известного аллокатора. \n
...
/// \brief \n Инструкция сама является вызовом известного аллокатора (malloc/new/HeapAlloc/...). \n
...
/// \brief \n Адрес целевой функции для thunk (алиаса). \n
...
/// \brief \n Человекочитаемое имя целевой функции thunk. \n
/// \details Для удобства отображения. Может быть пустой строкой, если имя нельзя получить.
/// \n\n
/// \ingroup FUNCTION_W ADR_RANGE_W
		std::string thunk_target_name;


/// \brief \n Количество всех чанков функции (head + tails). \n
...
/// \brief \n Количество хвостовых чанков (без головы). \n
/// \details tails_count = (chunks_total > 0 ? chunks_total - 1 : 0).
		uint32_t tails_count = 0;


/// \brief \n Суммарный размер кода по всем чанкам, байт. \n
...
/// \brief \n Демангленное (человекочитаемое) имя целевой функции thunk. \n
...
/// \brief \n Простая статистика по анализу функций/чанков. \n
///
...
/// \brief \n Вспомогательное множество для отслеживания уже встреченных head'ов. \n
...
/// \n\n
/// \ingroup ADR_RANGE_W FUNCTION_W
...
/// \brief \n устанавливает (добавляет) флаги в параметр func_flags , указанные функцией ::SetFunctionFlags() \n
/// \n
/// \param value
...
/// \brief \n      Вернуть количество функций в карте ... \n
...
/// \brief \n определяет флаги , которые нужно установить для функции \n
/// \n
...
/// \brief \n Сброс статистики Exporter::stats_. \n
/// \details Устанавливает все счётчики в ноль и очищает множество seen_heads.
	void ResetStats();


/// \brief \n Зарегистрировать классификацию одного чанка/функции. \n
/// \n
...
/// \brief \n Вывести сводную статистику через msg(). \n
...
/// \brief \n Разбор кадровой структуры (frame) функции. \n
...
/// \n\n
/// \ingroup FUNCTION_W ADR_RANGE_W SEARCH_W
...
/// \brief \n Статическая PE-информация о стеке процесса/потоков по умолчанию. \n
...
/// \brief \n OptionalHeader.SizeOfStackReserve, байт. \n
...
/// \brief \n OptionalHeader.SizeOfStackCommit, байт. \n
...
/// \brief \n Вернуть закэшированную PE-информацию о стеке. \n
...
/// \brief \n Перечитать статическую PE-информацию о стеке из заголовка файла. \n
/// Перечитать OptionalHeader.SizeOfStackReserve/Commit .
...
/// \brief \n Доступ/создание агрегатов по адресу функции. \n
/// \n\n
/// \ingroup FUNCTION_W
	FunctionEffects& GetOrCreateFuncEffects(ea_t fva);


/// \brief \n Найти агрегаты по адресу функции; nullptr, если нет. \n
/// \n\n
/// \ingroup FUNCTION_W SEARCH_W
	const FunctionEffects* FindFuncEffects(ea_t fva) const;
	// [EFFECTS] --- end ---

	// [EFFECTS+DERIVED] --- begin ---

/// \brief \n Завершение расчётов по эффектам: заполнить производные поля для всех функций. \n
...
private:
	...
	Stats stats_; ///< Runtime statistics for functions/chunks/thunks
				  // [STACK] --- begin ---
	PeImageInfo pe_image_info_; ///< \brief \n Кэш статических полей стека из PE OptionalHeader. \n
								// [STACK] --- end ---
								// [EFFECTS] --- begin ---
