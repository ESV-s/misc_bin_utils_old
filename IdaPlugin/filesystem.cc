...
#ifdef _WIN32
	/**
	 * \brief \n Предположим, что path является каталогом
	 */
...
absl::Status RemoveAll(absl::string_view path) {
	std::string path_copy(path);
#ifdef _WIN32
	// Связывание с файловой системой затруднено в Linux и macOS' XCode его не имеет.
	// В стандартную библиотеку Visual Studio она встроена.
	// TODO(cblichmann): Use filesystem once libstdc++ is >= 8.3 everywhere.
	namespace fs = std::experimental::filesystem;
	std::error_code ec;
	if (fs::remove_all(path_copy, ec) == static_cast<std::uintmax_t>(-1)) {
		return absl::UnknownError(
